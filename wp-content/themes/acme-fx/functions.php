...
add_filter( 'show_admin_bar', 'th_private_admin_bar' );
/** '__return_false' **/
...
//	if ( !is_super_admin() || !is_admin_bar_showing() ) return;
	if ( ! ( current_user_can( 'manage_options' ) || current_user_can( 'manage_woocommerce' ) ) || ! is_admin_bar_showing() ) {
		return;
	}
...
// Add support for custom header -- original custom header from core (it's the_... not th_custom....)
...
function th_custom_footer() {
	?>
    <p>Copyright &copy; <?php echo date( 'Y' ); ?> <a href="<?php echo esc_url( home_url( '/' ) ); ?>">Acme FX</a></p>
    <span class="footer-middot">&nbsp; &middot; &nbsp;</span><p>Built by <a href="https://parkdalewire.com"
                                                                            target="_blank">Parkdale Wire</a></p>
	<?php
}
	...
	if ( is_product() && ( strpos( $departments, 'Rentals' ) !== false ) ) { ?>
        ...
        <table class="pricing-table">
			...
			<?php if ( $weekly_rental_price ) : ?>
                <tr class="<?php if ( ! $daily_rental_price > 0 || $monthly_rental_price > 0 ) {
					echo "border-row";
				} ?>">
                    <th>
                        <em>Weekly</em>
			...
			<?php if ( $monthly_rental_price ) : ?>
                <tr class="<?php if ( ! $daily_rental_price > 0 && ! $weekly_rental_price > 0 ) {
					echo "border-row";
				} ?>">
                    <th>
                        <em>Monthly</em>
...
add_shortcode( 'autoplay-video', 'th_inline_video_shortcode' );
/**
 * Displays video with these attributes: playsinline, muted, autoplay, loop
 *
 * @param $atts
 *
 * @return false|string
 ...
 *
 */
function th_inline_video_shortcode( $atts ) {

	foreach ( $atts as $param_value ) {
		if ( false !== strpos( $param_value, 'http' ) ) {
			$atts['src'] = $param_value;
		}
	}
	...
	foreach ( $extended_atts as $param => $value ) {
